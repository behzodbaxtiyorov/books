...
const AuthorCard = (props: AuthorCardProps) => {
  return (
    <div className="w-[190px] p-[10px] rounded-lg shadow-md cursor-pointer hover:shadow-lg border-t-[7px] duration-500">
        ...
        <div>
           <h3 className="fon-crimson leading-normal text-[22px] my-[7px] text-[#C9AC8C] text-center">
         {`${props?.first_name + ' ' + props?.last_name}` ?.length > 13 ? `${props?.first_name + ' ' + props?.last_name}`.slice(0, 13) + "..." : `${props?.first_name + ' ' + props?.last_name}`}
           </h3>
           <p className="font-crimson text-center">
             {props?.data_brith} - {props?.data_death}
           </p>
        </div>
    </div>
  )
